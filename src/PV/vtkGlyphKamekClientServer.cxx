// ClientServer wrapper for vtkGlyphKamek object
//
#define VTK_WRAPPING_CXX
#define VTK_STREAMS_FWD_ONLY
#include "vtkSystemIncludes.h"
#include "vtkGlyphKamek.h"
#include "vtkClientServerInterpreter.h"
#include "vtkClientServerStream.h"


vtkObjectBase *vtkGlyphKamekClientServerNewCommand()
{
  return vtkGlyphKamek::New();
}

int vtkPolyDataAlgorithmCommand(vtkClientServerInterpreter*, vtkObjectBase*, const char*, const vtkClientServerStream&, vtkClientServerStream& resultStream);

int VTK_EXPORT vtkGlyphKamekCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream)
{
  vtkGlyphKamek *op = vtkGlyphKamek::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkGlyphKamek.  "
  ...
  if (!strcmp("SetSource",method) && msg.GetNumberOfArguments(0) == 4)
    {
    int      temp0;
    vtkPolyData  *temp1;
    if(msg.GetArgument(0, 2, &temp0) &&
  ...
  if (vtkPolyDataAlgorithmCommand(arlu, op,method,msg,resultStream))
    {
    return 1;
    }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkGlyphKamek, could not find requested method: \""
...
void vtkTransform_Init(vtkClientServerInterpreter* csi);
void vtkDataSet_Init(vtkClientServerInterpreter* csi);
void vtkPolyData_Init(vtkClientServerInterpreter* csi);
void vtkAlgorithmOutput_Init(vtkClientServerInterpreter* csi);
void vtkObject_Init(vtkClientServerInterpreter* csi);
void vtkPolyDataAlgorithm_Init(vtkClientServerInterpreter* csi);

//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkGlyphKamek_Init(vtkClientServerInterpreter* csi)
{
  ...
  if(last != csi)
    ...
    csi->AddNewInstanceFunction("vtkGlyphKamek", vtkGlyphKamekClientServerNewCommand);
    csi->AddCommandFunction("vtkGlyphKamek", vtkGlyphKamekCommand);
    }
}
